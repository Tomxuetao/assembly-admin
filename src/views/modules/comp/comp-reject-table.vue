<template>
  <div class="table-wrapper">
    ...
    <el-table row-key="applyId" :data="dataList" @selection-change="selectionChangeHandle" @sort-change="dataSortChangeHandle" ref="tableRef" v-loading="dataListLoading" height="715" style="width: 100%">
      ...
      <el-table-column label="组件名称" prop="serviceName" show-overflow-tooltip width="160"></el-table-column>
      ...
      <el-table-column label="审批状态" prop="applyStatusStr" width="180"></el-table-column>
      <el-table-column v-if="$isAuth(['comp:comp:detail', 'comp:comp:reapply'])" label="操作" align="center" width="160">
        <template #default="scope">
          <el-button v-if="$isAuth('comp:comp:detail')" type="text" size="small" @click="gotoDetailHandle(scope.row)">详情</el-button>
          <el-button v-if="$isAuth('comp:comp:reapply')" type="text" size="small" @click="reapplyAppHandle(scope.row)">重新申请</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  setup() {
    ...
    const gotoDetailHandle = ({ applyId }) => {
      router.push({ name: 'comp-detail', query: { id: applyId, status: 3 }})
    }

    const reapplyAppHandle = ({ id, serviceMode }) => {
      ElMessageBox.confirm('被驳回的申请,系统支持在原有申请单基础上重新编辑并提交审批。点击确定，重新调起流程。', '请确认是否要重新编辑并发起该申请', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        router.push({ name: 'release-index', query: { id, type: 'reapply', mode: serviceMode } })
      })
    }
