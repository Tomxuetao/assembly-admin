<template>
  <div class="approve-wrapper">
    ...
    <div class="table-operate">
      <div>
        <el-button v-if="$isAuth('approve:dept:export')" size="default" @click="exportDataListHandle()" style="width: 88px; margin: 0;">导 出</el-button>
      </div>
    </div>
      ...
      <el-table :data="dataList" ref="tableRef" row-key="id" @selection-change="selectionChangeHandle" v-loading="dataListLoading" height="650">
        ...
        <el-table-column label="审批状态" prop="approvalStatus" width="120">
          <template #default="scope">
            ...
            <template v-else>
              <el-tooltip placement="top" :content="`${scope.row.approverName} ${scope.row.approvalTime}`">
                <span>已审批</span>
              </el-tooltip>
            </template>
          </template>
        ...
        <template v-if="activeStatus !== 0">
          <el-table-column label="审批结果" prop="approvalStatusStr" width="220"></el-table-column>
        </template>
        <el-table-column v-if="$isAuth('approve:dept:detail')" label="操作" width="80">
          <template #default="scope">
            <el-button type="text" size="small" @click="router.push({ name: 'approve-dept-detail', query: { id: scope.row.id } })">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { getDeptApproveListApi, exportApproveListApi, exportApproveListByIdsApi } from '@/api/approve-api'
...
export default {
  ...
  setup() {
    ...
    const getDataListHandle = (page = { pageNum: 1, pageSize: 10 }) => {
      ...
      searchForm.approvalStatus = activeStatus.value
      getDeptApproveListApi(Object.assign(paramsForm, page), searchForm).then(({ data }) => {
    ...
    const exportDataListHandle = () => {
      ...
      const dataForm = selectedLen ? { queryType: 0 } : { approvalStatus: activeStatus.value, queryType: 0 }
